...
const BloqueInput = db.bloqueinputDin;
const BloqueInput2 = db.bloqueinputDin2;

exports.create = (req, res) => {
	console.log("Creando un bloque input"+req.body.next_id);
	BloqueInput.create({
...
exports.findAll = (req, res) => {
	BloqueInput2.findAll().then(bloqueinputdinamico => {
			// Send All Customers to Client
			res.json(bloqueinputdinamico.sort(function(c1, c2){return c1.id - c2.id}));
		}).catch(err => {	
			console.log(err);
			res.status(500).json({msg: "error", details: err});
		});
